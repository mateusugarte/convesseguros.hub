      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            onClick={onMove}
            className="btn-primary text-xs"
            disabled={!target || count === 0 || (target === 'enviado_cobranca' && !canConfirmCobranca)}
          >
            Mover
          </button>
        </div>
...
export default function Relatorio() {
  ...
  const imobiliariasRef = useRef(imobiliarias)
  useEffect(() => { imobiliariasRef.current = imobiliarias }, [imobiliarias])
  ...
  useEffect(() => {
    ...
    async function loadRows() {
      ...
      try {
        ...
        if (baseIds.size === 0 && extraIds.length === 0) {
          setRows([])
          return
        }

        // Fichas emitidas fora do filtro de status/periodo (extraIds) precisam
        // ser buscadas a parte; as que ja vieram em createdRows nao precisam
        // ser buscadas de novo (evita refetch duplicado das mesmas fichas).
        ...
        const fichaRows = extraIds.length > 0
          ? [...baseRows, ...(extraRows || [])].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))
          : baseRows
  ...
  async function moveSelected() {
    ...
    if (selectedRows.some(item => !MANUAL_REPORT_MOVE_OPTIONS.some(opt => opt.id === getColuna(item)))) {
    ...
    if (moveTarget === 'aprovada' && selectedRows.some(item => (
      getColuna(item) === 'expirada' &&
      isFichaExpiredOperational({ ...item, raw_data: { ...(item.raw_data || {}), manually_expired: false } })
    ))) {
      toast({ type: 'info', title: 'Não é possível restaurar', message: 'Ficha(s) com mais de 45 dias não podem voltar para Aprovadas — o prazo já passou de fato.' })
      return
    }
