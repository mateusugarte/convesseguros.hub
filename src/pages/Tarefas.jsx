...
import {
  ...
  RotateCcw, Search, Trash2, UserRound, UsersRound, X,
} from 'lucide-react'
              ...
              <div className="task-link-results">
                {results.map(result => (
                  <button key={`${result.entity_source}-${result.entity_id}`} type="button" onClick={() => addLink(result)}>
                    ...
                    <span><strong>{result.entity_label}</strong><small>{result.entity_detail}</small></span>
                    <Plus />
                  </button>
                ))}
              </div>
            ...
            <div className="task-selected-links">
              {selectedLinks.length === 0 ? (
                ...
                <div key={`${link.entity_source}-${link.entity_id}`}>
                  {link.entity_type === 'policy' ? <FileCheck2 /> : <UserRound />}
                  <span><strong>{link.entity_label}</strong><small>{link.entity_detail}</small></span>
  ...
  return (
    <div className="tasks-page">
      <section className="tasks-hero">
        <div>
          <div className="tasks-kicker"><ListChecks /> Organização pessoal</div>
          <h1>Minhas tarefas</h1>
          <p>Planeje o dia, registre contexto e mantenha seus prazos visíveis em todo o sistema.</p>
        </div>
        <div className="tasks-hero-actions">
          ...
          <Button onClick={() => setEditor({ task_date: selectedDate, owner_id: selectedOwner || user.id })} iconLeft={profile?.is_admin && !viewingSelf ? <UsersRound /> : <Plus />}>
      ...
      <section className="tasks-summary">
        <div><span className="is-blue"><ListChecks /></span><div><strong>{pending.length}</strong><small>Pendentes no dia</small></div></div>
        <div><span className="is-red"><AlarmClock /></span><div><strong>{alertCount}</strong><small>Prazos em atenção</small></div></div>
        <div><span className="is-green"><CheckCircle2 /></span><div><strong>{completed.length}</strong><small>Concluídas no dia</small></div></div>
      </section>
      ...
      <div className="tasks-layout">
        <main className="tasks-list-panel">
          <header>
            <div>
              <span>{isToday(parseDateKey(selectedDate)) ? 'Hoje' : format(parseDateKey(selectedDate), 'EEEE', { locale: ptBR })}</span>
