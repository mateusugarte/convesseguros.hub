  ...
  return (
    <div className="auto-page space-y-6 animate-fade-in">
      <PageHeader
        ...
        stats={(
          <>
            ...
            <MetricCard label="Comissão" value={formatMoney((Number(form.premio_liquido) || 0) * (Number(form.pct_comissao) || 0))} hint="estimada na emissão" tone="warning" icon={<FileText className="h-4 w-4" />} />
          </>
        )}
      />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <DataCard title="Dados da emissão"><div className="grid gap-3 md:grid-cols-2"><ReadItem label="ID da emissão" value={apolice.emissoes_auto?.id} /><ReadItem label="Resultado" value={apolice.emissoes_auto?.resultado || 'Em andamento'} /><ReadItem label="Criada em" value={formatDateTimeBR(apolice.emissoes_auto?.created_at)} /><Field label="Origem do lead" value={form.origem_lead} onChange={value => setForm(current => ({ ...current, origem_lead: value }))} /></div></DataCard>
