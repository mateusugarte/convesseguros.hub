...
export default function AutoEmissoesPlanilha() {
  ...
  const [showPolicyImport, setShowPolicyImport] = useState(() => searchParams.get('subir') === '1')
  ...
  const togglePolicyImport = () => {
    const next = !showPolicyImport
    setShowPolicyImport(next)
    const params = new URLSearchParams(searchParams)
    if (next) params.set('subir', '1')
    else params.delete('subir')
    setSearchParams(params, { replace: true })
  }
  const closePolicyImport = () => {
  ...
  return <div className="auto-page auto-operation-page animate-fade-in">
    <PageHeader eyebrow="Operação Auto · Apólices" title="Planilha de emissões" description="Edite a produção diretamente, cole blocos completos e suba apólices com revisão antes de gravar." actions={<div className="flex flex-wrap gap-2"><input className="input" type="month" value={month} onChange={event => setMonth(event.target.value || currentMonthRef())} /><button className="btn-secondary" onClick={() => navigate('/auto/emissoes')}>Voltar para apólices</button><button className="btn-secondary" onClick={() => setShowComposer(value => !value)}><Plus className="h-4 w-4" />Nova linha</button><button className="btn-primary" onClick={togglePolicyImport}><FileUp className="h-4 w-4" />Subir apólices</button></div>} />
    {showPolicyImport && <AutoPolicyImportSheet onClose={closePolicyImport} />}
