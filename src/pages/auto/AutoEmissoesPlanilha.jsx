...
import { Download, ExternalLink, FilePlus2, FileSpreadsheet, Plus, Search, ShieldCheck, X } from 'lucide-react'
import {
...
function exportCsv(rows) {
  const headers = ['Transmissão', 'Vigência', 'Segurado', 'Veículo', 'Placa', 'Parcelas', 'Seguradora', 'Prêmio líquido', '% comissão', 'Valor comissão', 'Repasse', 'Corretor', 'Tipo', 'Emissor', 'Status']
  const values = rows.map(row => [row.data_transmissao, row.vigencia_inicio, nameFor(row), row.modelo_veiculo, row.placa, row.parcelamento, row.seguradora, row.premio_liquido, row.pct_comissao, row.valor_comissao, row.valor_repasse, row.responsavel, row.cotacoes_auto?.tipo || row.tipo, row.emissor, getEmissaoColuna(row)])
...
export default function AutoEmissoesPlanilha() {
  ...
  const columns = useMemo(() => [
    ...
    { field: 'nome_cliente', label: 'Segurado', editable: true, sortable: true, width: 220, getValue: nameFor },
